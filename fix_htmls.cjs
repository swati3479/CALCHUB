...
if (firstFunc !== -1 && gstFunc !== -1) {
  code = code.substring(0, firstFunc) + '\n' + makeCode() + '\n' + code.substring(gstFunc);
  fs.writeFileSync('script_fixed3.js', code);
  console.log("Replaced HTML generators successfully!");
} else {
  console.log("Could not find delimiters", firstFunc, gstFunc);
}
